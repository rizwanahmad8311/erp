...
class TestAppRegistry:
    ...
    def test_accounting_holds_the_two_ledgers_and_nothing_from_the_business(self):
        ...
        Two more earn their place, and each for a reason that does not extend to
        anything else:

        * ``FiscalYearClose`` — the year-end closing entry. It writes to the
          ledger, so CLAUDE.md §5 says it is a ``DocumentModel``; and what it
          closes is the chart, which lives here. It is the only *document* in
          this app, and a second one would mean the business had leaked in.
        * ``IntegrityRun`` — the result of ``check_integrity``. A record of an
          observation *about* the ledger, not a financial record: it is not
          append-only, holds no money, and pruning it costs nobody anything.

        An invoice, a payment or a client appearing here is the failure this
        guards against — those belong to the apps that own them, and accounting
        must stay the layer they all post *into*.
    ...
    def test_the_year_end_close_is_the_only_document_in_accounting(self):
        """The guard that keeps the business out of the ledger layer.

        Stated separately from the set above so that the reason survives: a
        second document here would mean invoices, bills or payments had started
        migrating into the app every one of them posts into.
        """
        from django.apps import apps as django_apps
        ...
        documents = sorted(
            m.__name__
            for m in django_apps.get_app_config("accounting").get_models()
            if issubclass(m, DocumentModel)
        )
        assert documents == ["FiscalYearClose"], (
            f"accounting should hold one document, the year-end close; found {documents}"
        )

    def test_the_integrity_log_holds_no_money(self):
        """It records an observation, not a figure. A paisa value here would be
        a second place money lives, and it would not be the ledger (§6)."""
        from apps.accounting.models import IntegrityRun

        banned = ("paisa", "amount", "balance", "total")
        offenders = [
            f.name
            for f in IntegrityRun._meta.get_fields()
            if getattr(f, "attname", None) and any(word in f.name for word in banned)
        ]
        assert not offenders, f"IntegrityRun must hold no monetary figure: {offenders}"
